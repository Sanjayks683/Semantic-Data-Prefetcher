"""
test_simulator.py - Unit tests for the shared simulation loop, in particular the
prefetch latency model, whose off-by-one behaviour decides every latency result.
"""
...
class Scripted:
    """A prefetcher that issues a fixed list of addresses on chosen accesses."""

    def __init__(self, plan):
        self.plan = plan      # {access index: [addresses]}
        self.n = 0
...
class SingleOutput:
    """Old-style prefetcher exposing only access(), to test the fallback."""
...
def blocks(*indices):
    """Distinct addresses in distinct cache sets, so nothing evicts anything."""
...
def stream_with_target_at(position, length):
    """`length` distinct filler accesses with the target block at `position`."""
...
def test_latency_blocks_use_until_arrival():
    latency = 5
    # Issued on access 0, so it arrives at access 0 + 1 + 5 = 6.
...
def test_late_prefetch_is_not_later_inserted():
    """A cancelled prefetch must not arrive afterwards and pose as a prefetched line."""
    latency = 4
    s = stream_with_target_at(2, 20)
    s[15] = (IP, TARGET)          # second demand access, well after arrival time
...
def test_cancelled_prefetch_cannot_arrive_after_eviction():
    """The case that actually exposes a missing cancellation.

    If the demand-fetched line is still resident when the late prefetch lands,
    Cache.insert() quietly ignores it, which masks the bug. So evict the line
    before arrival: an uncancelled prefetch would then land as a fresh
    'prefetched' line and a later demand hit would be wrongly credited to it.
    """
    ...
    s = (blocks(0, 1)
         + [(IP, TARGET)]            # access 2: demand miss while in flight -> late
         + same_set                  # accesses 3..10: fill the set, evicting TARGET
         + blocks(*range(2, 16))     # accesses 11..24: prefetch would land at 21
         + [(IP, TARGET)])           # access 25: demand again
    m = simulate(s, prefetcher=Scripted({0: [TARGET]}), latency=latency)
    assert m.late_prefetches == 1
...
def test_resident_block_is_filtered_under_latency():
    s = blocks(*range(6))
    s[0] = (IP, TARGET)           # demand fill makes the target resident first
...
def test_l2_latency_uses_the_global_access_clock():
    """L1 hits consume time too, so arrival is measured in all accesses."""
    latency = 3
    # Repeat one block so accesses 1..3 are L1 hits that never reach L2.
    ...
    # Issued at access 0, arrives at 4 by the global clock: exactly when needed.
    assert m.l1_hits == 3
