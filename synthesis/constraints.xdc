...
#
# Ports are selected by direction rather than listed by name, so every output is
# constrained on both cores - including access_done, which only the pipelined
# core (ngram_prefetcher_pipe) has. A hard-coded list would leave it untimed.
